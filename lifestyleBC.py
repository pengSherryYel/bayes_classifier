...
def runHmmsearch(inputfile,prefix,wd,hmmModel,otherPara="--cpu 1"):
    ...
    checkEnv("hmmsearch")
    mkdirs(wd)    
    cmd = "hmmsearch {4} -o {2}/{1}.hmmsearch.out --tblout {2}/{1}.hmmsearch.tblout {3} {0}".format(inputfile,prefix,wd,hmmModel,otherPara)
    print("RUN command: %s\n"%cmd)
    obj = Popen(cmd,shell=True)
    obj.wait()
...
def runMmseqsEasysearch(inputfile,prefix,wd,hmmDB,otherPara="-s 7 --max-seqs 1 --alignment-mode 3 --alignment-output-mode 0 --min-aln-len 40  --cov-mode 0 --greedy-best-hits 1 --threads 30"):
    ...
    cmd = "mmseqs easy-search {0} {1} {2} {3} {4} && rm -rf {3} && echo 'mmesqs search done!'".format(inputfile,hmmDB,output,tmpdir,otherPara)
    print("RUN command: %s\n"%cmd)
    obj = Popen(cmd,shell=True)
    obj.wait()
    return output
...
def bayes_classifier_single(inputfile,prefix,wd,pfam_creteria=1e-5,mmseqs_creteria=1e-5):
    ...
    inte_opt = runHmmsearch(inputfile,"%s.BC_integrase"%prefix,pfam_wd,integrase_hmm) 
    excision_opt = runHmmsearch(inputfile,"%s.BC_excisionase"%prefix,pfam_wd,excisionase_hmm)
    ...
    mmseq_opt = runMmseqsEasysearch(inputfile,mmseqs_prefix,mmseqs_wd,bc_mmseqsDB,otherPara="-s 7 --max-seqs 1 --alignment-mode 3 --alignment-output-mode 0 --min-aln-len 40  --cov-mode 0 --greedy-best-hits 1 --threads 30")
    #print(mmseq_opt)
    ...
    p_total_temperate,p_total_lytic,bc_label = calcaulate_score(mmseq_opt,member2scoreD,creteria=mmseqs_creteria)    
    print(prefix,inte_label,excision_label,pfam_label,p_total_temperate,p_total_lytic,bc_label)
...
def bayes_classifier_batch(inputfile,wd,summaryfile="BC_predict.summary",pfam_creteria=1e-5,mmseqs_creteria=1e-5):
    ...
    with open(inputfile) as f:
        for line in f:
            ...
            res = bayes_classifier_single(path,sample_name,wd,pfam_creteria,mmseqs_creteria) 
